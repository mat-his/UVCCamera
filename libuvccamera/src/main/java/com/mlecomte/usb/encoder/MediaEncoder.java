package com.mlecomte.usb.encoder;
...
//		android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_URGENT_AUDIO);
		...
		// 创建h264
		FileUtilsManager.createfile(Environment.getExternalStorageDirectory().getAbsolutePath()+"/test222.h264");
                ...
                if (mBufferInfo.size != 0) {
					...
					if(mTrackIndex == 0) {
						...
						if (sync) {
							...
							// 保存数据流到文件
							FileUtilsManager.putFileStream(h264, 0,mPpsSps.length + mBufferInfo.size);
						} else {
							...
							FileUtilsManager.putFileStream(h264, 0,mBufferInfo.size);
						}
